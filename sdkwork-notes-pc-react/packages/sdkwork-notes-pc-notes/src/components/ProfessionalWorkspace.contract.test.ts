...
describe('professional notes workspace contracts', () => {
  it('renders a workspace overview grid with metric cards and a current-focus panel', () => {
    const insightsSource = read('packages/sdkwork-notes-pc-notes/src/components/NotesWorkspaceInsightsPanel.tsx');
  ...
  it('renders document insights in the editor pane and structure controls in the inspector', () => {
    ...
    const inspectorSource = read('packages/sdkwork-notes-pc-notes/src/components/NoteInspectorPanel.tsx');
    const pageSource = read('packages/sdkwork-notes-pc-notes/src/pages/NotesWorkspacePage.tsx');
    const commandPaletteSource = read('packages/sdkwork-notes-pc-notes/src/components/NoteCommandPalette.tsx');
    const hotkeySource = read('packages/sdkwork-notes-pc-notes/src/services/noteWorkspacePageActions.ts');
