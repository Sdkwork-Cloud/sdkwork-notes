...
describe('desktop theme consistency contracts', () => {
  ...
  it('scans the workspace root so desktop css includes lazy auth and notes module utilities', () => {
    const shellStylesPath = path.resolve(
      workspaceRoot,
      'packages/sdkwork-notes-pc-shell/src/styles/index.css',
    );
  ...
  it('keeps theme-sensitive pages on semantic tokens instead of raw light or zinc colors', () => {
    ...
    const authCallbackPageSource = read('packages/sdkwork-notes-pc-auth/src/pages/AuthOAuthCallbackPage.tsx');
    const identityThemeProviderSource = read(
      'packages/sdkwork-notes-pc-auth/src/theme/SdkworkIamThemeProvider.tsx',
    );
    const notesSidebarSource = read('packages/sdkwork-notes-pc-notes/src/components/NotesSidebar.tsx');
    const noteEditorPaneSource = read('packages/sdkwork-notes-pc-notes/src/components/NoteEditorPane.tsx');
