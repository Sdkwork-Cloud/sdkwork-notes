# sdkwork-notes-backend-sdk (Rust)
...
```bash
cargo add sdkwork-notes-backend-sdk-generated
```

## Quick Start

```rust
use sdkwork_notes_backend_sdk_generated::{SdkworkBackendClient, SdkworkConfig};
...
client.set_access_token("your-access-token");
    ...
    let result = client.notes().workspaces_admin_list(Some(&query)).await?;
    println!("{result:?}");
    Ok(())
}
```
...
let outcome: Result<(), _> = async {
    ...
    query.insert("page_size".to_string(), serde_json::json!(2));
    query.insert("q".to_string(), serde_json::json!("q"));
    client.notes().workspaces_admin_list(Some(&query)).await?;
    Ok(())
}.await;
