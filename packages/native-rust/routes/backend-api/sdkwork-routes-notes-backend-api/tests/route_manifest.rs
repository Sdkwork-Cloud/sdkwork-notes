...
fn backend_route_manifest_matches_backend_openapi_ai_job_operations() {
    ...
    assert_eq!(manifest["packageName"], "sdkwork-routes-notes-backend-api");
    assert_eq!(manifest["surface"], "backend-api");
    assert_eq!(manifest["owner"], "sdkwork-notes");
    assert_eq!(manifest["domain"], "notes");
    assert_eq!(manifest["apiAuthority"], "sdkwork-notes-backend-api");
    assert_eq!(manifest["sdkFamily"], "sdkwork-notes-backend-sdk");
    ...
    let expected_operations = BTreeSet::from([
        "GET /backend/v3/api/notes/ai_jobs".to_string(),
        "GET /backend/v3/api/notes/ai_jobs/{aiJobId}".to_string(),
        "POST /backend/v3/api/notes/ai_jobs/{aiJobId}/cancel".to_string(),
        "POST /backend/v3/api/notes/ai_jobs/{aiJobId}/claim".to_string(),
        "POST /backend/v3/api/notes/ai_jobs/{aiJobId}/complete".to_string(),
        "POST /backend/v3/api/notes/ai_suggestions/{aiSuggestionId}/accept".to_string(),
        "POST /backend/v3/api/notes/ai_suggestions/{aiSuggestionId}/reject".to_string(),
        "POST /backend/v3/api/notes/ai_suggestions/{aiSuggestionId}/apply".to_string(),
        "GET /backend/v3/api/notes/ai_suggestions/{aiSuggestionId}/feedback".to_string(),
    ]);
...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(5)
        .expect("route crate should live under packages/native-rust/routes/backend-api")
        .to_path_buf()
}
