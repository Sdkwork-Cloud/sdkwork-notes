...
当前主工程 `sdkwork-notes-pc-react` 采用 `pnpm workspace` 的多包结构。

### 3.2.1 当前包职责矩阵

| 包名 | 角色 | 主要职责 | 当前评价 |
|---|---|---|---|
| `@sdkwork/notes-shell` | 应用壳 | 路由、布局、Provider、主题/语言集成 | 设计清晰 |
| `@sdkwork/notes-auth` | 认证域 | 认证页面、认证控制器、认证主题、OAuth/二维码登录编排、app-sdk 认证桥接 | 边界良好 |
| `@sdkwork/notes-notes` | 笔记域 | 工作区页面、编辑器、侧栏、检视器、命令面板、仓储、选择器、布局状态 | 核心域，需持续拆细 |
| `@sdkwork/notes-user` | 用户域 | 账户页、资料与设置 | 边界清楚 |
| `@sdkwork/notes-core` | 共享核心 | app-sdk 客户端、会话编排、运行时配置解析、用户服务、应用状态 | 核心基础层 |
| `@sdkwork/notes-commons` | 共享组件/工具 | Button、Dialog、SurfaceCard、服务适配器结果 | 合理 |
| `@sdkwork/notes-i18n` | 国际化 | 语言资源、i18n 初始化 | 合理 |
| `@sdkwork/notes-types` | 类型层 | Note、Folder、Result、App 类型定义 | 合理 |
| `@sdkwork/notes-desktop` | 桌面壳 | Tauri 运行时、桥接、托盘、窗口控制、启动壳、启动外观快照、运行时信息投影、原生会话桥接 | 边界清楚 |
| `@sdkwork/notes-local` | 本地能力骨架 | 本地权威副本、草稿恢复、未来本地数据库适配入口 | Step 02 已建骨架，待 Step 06 兑现能力 |
| `@sdkwork/notes-search` | 搜索能力骨架 | 索引模型、统一查询语义、命令面板候选源入口 | Step 02 已建骨架，待 Step 07 兑现能力 |
| `@sdkwork/notes-sync` | 同步能力骨架 | 同步队列、状态机、重试与冲突恢复入口 | Step 02 已建骨架，待 Step 08 兑现能力 |
| `@sdkwork/notes-observability` | 观测能力骨架 | 日志、指标、事件封装入口 | Step 02 已建骨架，待 Step 10/11 继续深化 |
| `@sdkwork/notes-updater` | 更新能力骨架 | 桌面更新渠道、版本清单与更新策略入口 | Step 02 已建骨架，待 Step 09 兑现能力 |
