...
| `VITE_APP_ACCESS_TOKEN` / `VITE_ACCESS_TOKEN` | 接口访问令牌注入 |
| `VITE_TENANT_ID` | 租户上下文注入 |
| `VITE_ORGANIZATION_ID` | 组织上下文注入 |
...
- SDK client 当前会优先读取显式 `VITE_APP_OWNER_MODE / VITE_OWNER_MODE / SDKWORK_OWNER_MODE`，否则再根据租户/组织相关变量推断 `ownerMode = root / tenant / organization`，进而决定 `baseUrl / accessToken / tenantId / organizationId` 的作用域选择。
