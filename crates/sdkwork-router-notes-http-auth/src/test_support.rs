...
pub fn test_auth_claim_token(tenant_id: &str, organization_id: &str, user_id: &str) -> String {
    format!(
        "tenant_id={tenant_id};organization_id={organization_id};user_id={user_id};session_id=session-test;app_id=appbase;auth_level=password;permission_scope=notes.*,notes.backend.*"
    )
}

pub fn test_access_claim_token(tenant_id: &str, organization_id: &str, user_id: &str) -> String {
    format!(
        "tenant_id={tenant_id};organization_id={organization_id};user_id={user_id};session_id=session-test;app_id=appbase;environment=prod;deployment_mode=saas"
    )
}
