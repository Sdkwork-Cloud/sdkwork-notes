...
pub async fn ensure_notes_tenant_application_bootstrap() -> Result<(), String> {
    let app_root = resolve_notes_app_root();
...
fn resolve_notes_app_root() -> PathBuf {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .canonicalize()
        .unwrap_or_else(|_| PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../.."));
    let pc_react_root = repo_root.join("sdkwork-notes-pc-react");
...
mod tests {
    ...
    #[test]
    fn notes_app_root_resolves_to_pc_react_manifest_root() {
        let root = resolve_notes_app_root();
