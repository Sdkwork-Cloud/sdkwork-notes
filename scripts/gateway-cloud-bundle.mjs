...
import {
  NOTES_CLOUD_GATEWAY_CONFIGS,
  PLATFORM_CONFIG_BUNDLE_PROFILE,
  REPO_ROOT,
} from './lib/notes-topology.mjs';

const repoRoot = REPO_ROOT;
const manifestPath = path.join(repoRoot, 'sdkwork-notes-pc-react', 'sdkwork.app.config.json');
...
Bundle Notes-owned sdkwork-api-gateway route configs for cloud topology deployment.
...
async function createBundleContext(options) {
  ...
  const stageName = `sdkwork-notes-api-gateway-config-${version}`;
...
async function bundleCloudConfig(context) {
  ...
  for (const configName of NOTES_CLOUD_GATEWAY_CONFIGS) {
...
These TOML files configure sdkwork-api-gateway for Notes cloud topology.
Build and deploy the gateway binary from the sdkwork-api-gateway repository.

Included configs:
${NOTES_CLOUD_GATEWAY_CONFIGS.map((name) => `- configs/${name}`).join('\n')}
`;
